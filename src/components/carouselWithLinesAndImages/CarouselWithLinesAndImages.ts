...
class CarouselWithLinesAndImages extends CarouselWithLines {
  ...
  setSlideShow() {
    ...
    if (this.numberOfClicks % 2) {}
    const svgTimer = this.getUniqueWrapper().querySelector<HTMLElement>('.js-timer');

    svgTimer.addEventListener('click', () => this.startSlideShow());
  }
  ...
  startSlideShow() {
    let startIndex = this.getCurrentActiveIndex();
    if (this.activeIndex === 0) {
      startIndex = this.images.length;
    }
    const intervalId = setInterval(() => {
      this.setNextImage();
      if (this.activeIndex === startIndex - 1) {
